...
static BitBoardElement _pawnAttacks[2][64]; // Precomputed pawn attacks for each square
...
void GameState::addPawnBitboardMovesToList(std::vector<BitMove>& moves, const BitBoardElement bitboard, const int shift) {
...
void GameState::generatePawnMoveList(std::vector<BitMove>& moves, const BitBoardElement pawns, const BitBoardElement emptySquares, const BitBoardElement enemyPieces, char color) {
    if (pawns.getData() == 0)
        return;
...
void GameState::generateKnightMoves(std::vector<BitMove>& moves, BitBoardElement knightBoard, uint64_t occupancy) {
    knightBoard.forEachBit([&](int fromSquare) {
        BitBoardElement moveBitboard = BitBoardElement(KnightAttacks[fromSquare] & occupancy);
...
// Generate actual move objects from a bitboard
void GameState::generateKingMoves(std::vector<BitMove>& moves, BitBoardElement piecesBoard, uint64_t occupancy) {
    piecesBoard.forEachBit([&](int fromSquare) {
        BitBoardElement moveBitboard = BitBoardElement(KingAttacks[fromSquare] & occupancy);
        // Efficiently iterate through only the set bits
        moveBitboard.forEachBit([&](int toSquare) {
...
// Generate actual move objects from a bitboard
void GameState::generateBishopMoves(std::vector<BitMove>& moves, BitBoardElement piecesBoard, uint64_t occupancy, uint64_t friendlies)
{
    piecesBoard.forEachBit([&](int fromSquare) {
...
void GameState::generateRooksMoves(std::vector<BitMove>& moves, BitBoardElement piecesBoard, uint64_t occupancy, uint64_t friendlies)
{
    piecesBoard.forEachBit([&](int fromSquare) {
...
void GameState::generateQueensMoves(std::vector<BitMove>& moves, BitBoardElement piecesBoard, uint64_t occupancy, uint64_t friendlies)
{
    piecesBoard.forEachBit([&](int fromSquare) {
...
inline BitBoardElement generatePieceAttackList(
    const BitBoardElement pieces, 
    const BitBoardElement occupancy
) {
    BitBoardElement attacks;
    ...
    pieces.forEachBit([&](int fromSquare) {
        ...
        if (PIECE_TYPE == Knight) {
            ...
            attacks |= BitBoardElement(getBishopAttacks(fromSquare, occupancy.getData())); 
        }
        else if (PIECE_TYPE == Rook) {
            attacks |= BitBoardElement(getRookAttacks(fromSquare, occupancy.getData())); 
        }
        else if (PIECE_TYPE == Queen) {
            // Queen is rook + bishop combined
            attacks |= (BitBoardElement(getBishopAttacks(fromSquare, occupancy.getData())) |
                        BitBoardElement(getRookAttacks(fromSquare, occupancy.getData())));
        }
...
const BitBoardElement GameState::generatePawnAttacks(const BitBoardElement pawns, char color) {
...
bool GameState::isSquareAttacked(int square, char attackerColor, const BitBoardElement (&boards)[e_numBitboards]) {
	...
	BitBoardElement occ = boards[OCCUPANCY];
...
void GameState::filterOutIllegalMoves(std::vector<BitMove>& moves) {
	...
	moves.erase(std::remove_if(moves.begin(), moves.end(), [&](const BitMove& move) {
		...
		BitBoardElement tempBoards[e_numBitboards];
